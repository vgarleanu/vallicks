...
use crate::naked_std::sync::atomic::{AtomicUsize, Ordering};
use crate::naked_std::sync::mpsc::blocking::{self, SignalToken, WaitToken};
use crate::prelude::sync::{Mutex, MutexGuard};
...
impl<T> Packet<T> {
    pub fn new(capacity: usize) -> Packet<T> {
        Packet {
            ...
            lock: Mutex::new(State {
                ...
                buf: Buffer {
                    buf: (0..capacity + if capacity == 0 { 1 } else { 0 })
                        .map(|_| None)
                        .collect(),
                    start: 0,
                    size: 0,
    ...
    // time.
    // Unlike in stdlib, we do not yet support deadlines
